...
struct TreeNode
{
  QueryNodeType code;
  TreeNode * up;
  TreeNode (QueryNodeType c) : code(c), up(0) {}
  Schema sch;
};
...
void PrintSchedulTree(TreeNode * tn)
{
      ...
      if(0 == distinctAtts)
        {
          ...
          */
          if (sfNodes.size() > 0 and 0 == jNodes.size() and 0 == spNodes.size())
            {
              clog << "only select, and nothing else, combined with no function and no distinct, makes hopefully an easy case." << endl;
              // I'll probably need to reseat the schema variables so they can be found and used.
            }
        }
    }
  clog << endl;
